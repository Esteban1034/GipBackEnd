...
public class ProjectStatusReportController {
    ...
    @Autowired
    private NotasSemanaPSRService notasSemanaPSRService;
    ...
    @GetMapping({ "/projectstatusreports/getNotasSemanalFechaIFechaF/{fechaInicio}/{fechaFin}" })
    public List<NotasSemanaPSR> getNotasSemanal(@PathVariable String fechaInicio,@PathVariable String fechaFin) {
        LocalDate fechaI = this.stringToLocalDate(fechaInicio);
        LocalDate fechaF = this.stringToLocalDate(fechaFin);
        return this.notasSemanaPSRService.getNotasSemanalFechaInicialFechaFinal(fechaI,fechaF);
    }

    @GetMapping({ "/projectstatusreports/getNotasSemanal" })
    public List<NotasSemanaPSR> getNotasSemanal() {
        return this.notasSemanaPSRService.getNotaSemanalPsr();
    }


    @GetMapping({ "/projectstatusreports/getNotaSemanalHoy" })
    public NotasSemanaPSR getNotaSemanalHoy() {
        LocalDate fechaHoy = LocalDate.now(ZoneId.of("America/Bogota"));
        return this.notasSemanaPSRService.getNotaSemanalFechaCreacion(fechaHoy);
    }


    @PostMapping({ "/projectstatusreports/saveNotaSemanal" })
    public ResponseEntity<?> saveNotaSemanal(@RequestBody NotasSemanaPSR notasSemana) {
        Map<String, Object> response = new HashMap<>();
        LocalDate fechaHoy = LocalDate.now(ZoneId.of("America/Bogota"));
        notasSemana.setFechaCreacion(fechaHoy);
        this.notasSemanaPSRService.saveNotaSemanalPsr(notasSemana);

        response.put("mensaje", "las notas se han guardado satisfactoriamente.");
        return new ResponseEntity<Map<String, Object>>(response, HttpStatus.CREATED);
    }


    @PostMapping({ "/projectstatusreports/saveObservacionNotaSemanal" })
    public ResponseEntity<?> saveObservacionNotaSemanal(@RequestBody NotasSemanaPSR notasSemana) {
        Map<String, Object> response = new HashMap<>();
        LocalDate fechaHoy = LocalDate.now(ZoneId.of("America/Bogota"));
        notasSemana.setFechaObservacion(fechaHoy);
