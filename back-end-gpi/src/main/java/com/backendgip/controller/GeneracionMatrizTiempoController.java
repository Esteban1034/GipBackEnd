...
public class GeneracionMatrizTiempoController {
    ...
    @DeleteMapping({ "/generacionMatrizTiempo/matriz/{id}"})
    public ResponseEntity<?> deleteMatriz(@PathVariable Integer id){
        MatrizTiempo matrizTiempo = (MatrizTiempo) this.matrizTiempoRepository.findById(id).orElseThrow(() -> {
        ...
        for (GeneracionMatrizTiempo sprint : sprintsVersion) {
            if (!this.generacionmatriztiemposTipoRecursosRepository.findByGeneracionMatrizTiempoId(sprint.getId()).isEmpty()) {
                for (Generarmatriztiemposasignacionrecursos item : this.generacionmatriztiemposTipoRecursosRepository.findByGeneracionMatrizTiempo(sprint)) {
                        this.generacionmatriztiemposTipoRecursosRepository.delete(item);
                } 
            }
            this.generacionMatrizTiempoService.deleteGeneracionTiemposById(sprint.getId());
        }
        LogSistema log = new LogSistema();
            log.setAccion("DELETE");
            log.setFechaHora(new Date());
            log.setIdAccion(matrizTiempo.getId());
            log.setDescripcion(matrizTiempo.toString());
            log.setTabla(MatrizTiempo.class.toString());
            this.logService.saveLog(log);
            this.matrizTiempoService.deleteMatrizById(matrizTiempo);
            Map<String, Boolean> response = new HashMap();
            response.put("deleted", Boolean.TRUE);
            return ResponseEntity.ok(response);
    }
